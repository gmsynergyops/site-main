...
export const CancerPrevention = () => {
  const t = useTranslations("cancerPreventionPage");

  const icons = [
    <ShieldCheck key="tobacco" className="text-red-500" />,
    <Salad key="diet" className="text-green-600" />,
    <Dumbbell key="active" className="text-indigo-600" />,
    <CupSoda key="alcohol" className="text-yellow-600" />,
    <Sun key="sun" className="text-orange-500" />,
    <Syringe key="vaccine" className="text-blue-600" />,
    <HeartPulse key="screenings" className="text-pink-600" />
  ];
                ...
                <CardContent className="p-6 space-y-4">
                  <div className="flex gap-4 items-center">
                    <div className="w-10 h-10 rounded-full bg-white/80 flex items-center justify-center">
                      {icons[index] || <ShieldCheck className="text-red-500" />}
                    </div>
